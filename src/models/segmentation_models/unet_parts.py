...
class CodeFilter(nn.Module):
    def __init__(self, z_level_1_channel, z_level_2_channel,norm=nn.BatchNorm2d, if_SN=False):
        super(CodeFilter, self).__init__()
        if not if_SN:
...
class up(nn.Module):
    def __init__(self, in_ch_1, in_ch_2, out_ch, up_type='bilinear', dropout=None, norm=nn.BatchNorm2d, if_SN=False, activation=nn.ReLU):
        super(up, self).__init__()
        self.up_type = up_type
        if up_type == 'bilinear':
            self.up = nn.Upsample(scale_factor=2, mode=up_type, align_corners=True)
        elif up_type == 'Conv2':
            self.up = nn.ConvTranspose2d((in_ch_1 + in_ch_2) // 2, (in_ch_1 + in_ch_2) // 2, 2, stride=2)
        elif up_type == 'nearest':
            self.up = nn.Upsample(scale_factor=2)
        else:
           raise ValueError("Unknown up_type '{}'".format(up_type))
